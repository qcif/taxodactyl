process CONFIGURE_ENVIRONMENT {
    ...
    if [ ${params.sequences} != null ]; then echo 'export INPUT_FASTA_FILEPATH=${sequences_file}' >> env_vars.sh; fi
    if [ ${params.metadata} != null ]; then echo 'export INPUT_METADATA_CSV_FILEPATH=${metadata_file}' >> env_vars.sh; fi
    ...
    if [ ${params.ncbi_api_key} != null ]; then echo 'export NCBI_API_KEY=${params.ncbi_api_key}' >> env_vars.sh; elif [ ! -z "\${NCBI_API_KEY}" ]; then echo 'export NCBI_API_KEY=\${NCBI_API_KEY}' >> env_vars.sh; fi
