...
| `metadata`     | string  | 'metadata.csv'    | CSV file containing sample metadata. Required columns: `sample_id`, `locus`, `preliminary_id`. Optional columns include `country` (if unknown, leave empty; do not use `NA`) and `classification` (one of `animalia`, `plantae`, `fungi`, `chromista`, `bacteria`, `archaea`, `viruses`).  | Must be a valid CSV file path, no spaces, `.csv` extension, required columns.                 |
...
| `phylogeny_min_hit_identity`   | float   | 0.935    | Minimum hit identity (0-1) to be included in the phylogenetic tree.                              | Number 0–1. Default: 0.935.           |
| `phylogeny_min_seqs`  | int     | 20      | Minimum number of sequences to be included in the phylogenetic tree.                             | Integer ≥ 1. Default: 20.
| `phylogeny_max_seqs`  | int     | 50      | Maximum number of sequences to be included in the phylogenetic tree.                             | Integer ≥ 1. Default: 50.            |
| `phylogeny_species_max_seqs`  | int     | 3      | Maximum number of sequences per non-candidate species to be included into phylogenetic tree.                             | Integer ≥ 1. Default: 3.
| `phylogeny_candidate_max_seqs`  | int     | 5      | Maximum number of sequences per candidate species to be included into phylogenetic tree.                             | Integer ≥ 1. Default: 5.            |
...
| `app_data_dir` | string  | '${System.getProperty(''user.home'')}/.local/share/taxodactyl' | Directory for persistent application data (e.g. secrets / local vault data), mounted to `/var/lib/taxodactyl` in containers. | Must be a valid directory path. |
| `temp_root_dir`| string  | '${(System.getenv(''TMPDIR'') ?: System.getProperty(''java.io.tmpdir'') ?: System.getProperty(''user.home'')) + ''/taxodactyl_tmp''}'| Directory where temporary files will be saved. Uses `TMPDIR` when set, otherwise falls back to `${java.io.tmpdir}`, then `${user.home}`. In all cases `/taxodactyl_tmp` is appended to the selected base directory. | Must be a valid directory path.      |
